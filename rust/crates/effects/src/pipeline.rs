...
const MAJAN_GLITCH_SHADER_ID: &str = "majan-glitch";
const MAJAN_GLITCH_SHADER_SOURCE: &str = include_str!("shaders/majan_glitch.wgsl");
const MAJAN_GRADE_SHADER_ID: &str = "majan-grade";
const MAJAN_GRADE_SHADER_SOURCE: &str = include_str!("shaders/majan_grade.wgsl");
const MAJAN_VIGNETTE_SHADER_ID: &str = "majan-vignette";
const MAJAN_VIGNETTE_SHADER_SOURCE: &str = include_str!("shaders/majan_vignette.wgsl");
...
impl EffectPipeline {
    pub fn new(context: &GpuContext) -> Self {
        ...
        let shader_list: [(&str, &str); 4] = [
            ...
            (MAJAN_GRADE_SHADER_ID, MAJAN_GRADE_SHADER_SOURCE),
            (MAJAN_VIGNETTE_SHADER_ID, MAJAN_VIGNETTE_SHADER_SOURCE),
        ];
        let mut pipelines: HashMap<String, wgpu::RenderPipeline> = HashMap::new();
        for (shader_id, shader_source) in shader_list {
            let shader_module =
                context
                    .device()
                    .create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            let pipeline = context
                ...
                .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                    label: Some("effects-shader-pipeline"),
...
fn pack_effect_uniforms(
    ...
    let (expected, scalars, direction): (&[&str], [f32; 4], [f32; 2]) = match shader {
        GAUSSIAN_BLUR_SHADER_ID => {
            ...
            (
                &["u_sigma", "u_step", "u_direction"][..],
                [sigma, step, 0.0, 0.0],
                direction,
            )
        }
        MAJAN_GLITCH_SHADER_ID => {
            let amount = read_number_uniform(pass, "u_amount")?;
            let seed = read_number_uniform(pass, "u_seed")?;
            let slices = read_number_uniform(pass, "u_slices")?;
            let rgb_shift = read_number_uniform(pass, "u_rgb_shift")?;
            (
                &["u_amount", "u_seed", "u_slices", "u_rgb_shift"][..],
                [amount, seed, slices, rgb_shift],
                [0.0, 0.0],
            )
        }
        MAJAN_GRADE_SHADER_ID => {
